	...
	if (d) {
		...
		.success(
				function(result) {
					...
					if (obj[0].LoginResult == 'Succeed') {
						windows.location.href="../Web_Pages/HomePage.jsp";
					} else if (obj[0].LoginResult == 'Fail') {
						var c = "用户不存在或密码错误";
						$("#modal-login-name").parent().append(
								"<label class='lblerr'>" + c + "</label>");
						d = false
...
function reg() {
	...
	var b2 = $.trim($("#modal-reg-pwd2").val());
	var d = true;
	if (a == "") {
		var c = "请输入昵称";
...
$(function() {
	...
	$("#modal-reg-pwd2").focus(resetLoginErr);
	...
	$("#modal-reg-pwd").keypress(function(a) {
		if (a.which == "13" || a.which == "9") {
			$("#modal-reg-pwd2").focus().select();
			a.stopPropagation()
		}
	});

	$("#modal-reg-pwd2").keypress(function(a) {
		if (a.which == "13") {
			reg()
		}
	});
