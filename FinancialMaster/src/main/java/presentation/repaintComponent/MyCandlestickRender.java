 ...
 * {@link OHLCDataset}). The example shown here is generated by the
 * <code>CandlestickChartDemo1.java</code> program included in the JFreeChart
 * demo collection: <br>
 * <br>
 * <img src="../../../../../images/CandlestickRendererSample.png" alt=
 * "CandlestickRendererSample.png">
 * <P>
 * This renderer does not include code to calculate the crosshair point for the
 * plot.
 */
public class MyCandlestickRender extends AbstractXYItemRenderer
	...
	/**
	 * A flag that controls whether or not the renderer's outline paint is used
	 * to draw the outline of the candlestick. The default value is
	 ...
	 *            The candle width.
	 */
	public MyCandlestickRender(double candleWidth) {
	 ...
	 */
	public MyCandlestickRender(double candleWidth, boolean drawVolume, XYToolTipGenerator toolTipGenerator) {
		super();
	 ...
	 * width.<br>
	 * <code>WIDTHMETHOD_SMALLEST</code>: Checks the interval between each item,
	 * and uses the smallest as the available width.<br>
	...
	/**
	 * Returns the factor by which the available space automatically calculated
	...
	/**
	 * Sets the factor by which the available space automatically calculated for
	 ...
	 * close and sends a {@link RendererChangeEvent} to all registered
	 * listeners.
	 *
	 * @param paint
	 ...
	 * is used to draw the candlestick outline, and sends a
	 * {@link RendererChangeEvent} to all registered listeners.
	 *
	 * @param use
	 *            the new flag value.
	 ...
	 * data that the renderer will require (usually just one). This method will
	 * be called before the first item is rendered, giving the renderer an
	 * opportunity to initialise any state information it wants to maintain. The
	 * renderer can do nothing if it chooses.
	 *
	 * @param g2
	 *            the graphics device.
	 * @param dataArea
	...
	public boolean equals(Object obj) {
		...
		if (this.candleWidth != that.getCandleWidth()) {
			return false;
		}
		if (!PaintUtilities.equal(this.upPaint, that.getUpPaint())) {
			return false;
		}
		if (!PaintUtilities.equal(this.downPaint, that.getDownPaint())) {
			return false;
		}
		if (this.drawVolume != that.getDrawVolume()) {
			return false;
		}
		if (this.maxCandleWidthInMilliseconds != that.getMaxCandleWidthInMilliseconds()) {
			return false;
		}
		if (this.autoWidthMethod != that.getAutoWidthMethod()) {
			return false;
		}
		if (this.autoWidthFactor != that.getAutoWidthFactor()) {
			return false;
		}
		if (this.autoWidthGap != that.getAutoWidthGap()) {
			return false;
		}
		if (this.useOutlinePaint != that.getUseOutlinePaint()) {
			return false;
		}
		if (!PaintUtilities.equal(this.volumePaint, that.getVolumePaint())) {
			return false;
		}
