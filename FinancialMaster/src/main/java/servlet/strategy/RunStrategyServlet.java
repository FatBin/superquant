...
public class RunStrategyServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (profitPO profitPO : profitPOs) {
			data=data+"{'date':'"+profitPO.getDate()+
					"','profit':"+profitPO.getProfit()+"},";
		}
		data+="]";
...
//      String totalcost=request.getParameter("totalcost");//总成本
      ...
      for (int i = 0; i < size; i++) {
			...
			StrategyPO buyPO=new StrategyPO(
					per[0], per[2], per[3], 
					Double.parseDouble(buy[0]), Double.parseDouble(buy[1]), 
					Double.parseDouble(buy[2]), Double.parseDouble(buy[3]), 
					Double.parseDouble(buy[4]), Double.parseDouble(buy[5]), 
					Double.parseDouble(buy[6]), Double.parseDouble(buy[7]), 
					Double.parseDouble(buy[8]), Double.parseDouble(buy[9]), 
					Integer.parseInt(per[5]), Double.parseDouble(per[1]));
			arrayList_buy.add(buyPO);
			StrategyPO soldPO=new StrategyPO(
					per[0], per[2], per[3], 
					Double.parseDouble(sold[0]), Double.parseDouble(sold[1]), 
					Double.parseDouble(sold[2]), Double.parseDouble(sold[3]), 
					Double.parseDouble(sold[4]), Double.parseDouble(sold[5]), 
					Double.parseDouble(sold[6]), Double.parseDouble(sold[7]), 
					Double.parseDouble(sold[8]), Double.parseDouble(sold[9]), 
					Integer.parseInt(per[5]), Double.parseDouble(per[1]));
